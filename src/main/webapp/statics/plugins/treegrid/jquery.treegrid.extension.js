        ...
        target.renderRow = function(tr,item){
            $.each(options.columns, function(index, column) {
                ...
                if(index==0&&column.field=='selectItem'){
                    ...
                    var td = $('<td style="'+((column.width)?('width:'+column.width):'')+ ((column.display)?(';display:'+column.display):'')+ ((column.align)?(';text-align:'+column.align):'')+'"></td>');
                    // 增加formatter渲染
                    if (column.formatter) {
                        td.html(column.formatter.call(this, item, index, tr.index(this)));
                    } else {
        ...
        target.load = function(parms){
            ...
            $.each(options.columns, function(i, item) {
                ...
                if(i==0&&item.field=='selectItem'){
                    ...
                    th = $('<th style="padding:10px;'+((item.width)?('width:'+item.width):'')+ ';' +((item.display)?('display:'+item.display):'') + ';' +((item.align)?('align:'+item.align):'')+'"></th>');
                }
                th.text(item.title);
                thr.append(th);
            });
