...
$(function () {
    ...
    // 初始化插件参数及插入插件
    WebVideoCtrl.I_InitPlugin(1200, $(window).height() - 440, {
...
function clickRollBack(starttime){
...
function clickStartPlayback() {
	$("#divPlugin").removeAttr('style');
	$("#myPlayer").attr('style', 'display:none');
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
        szDeviceIdentify = $("#ip").val(),
        iRtspPort = parseInt($("#rtspport").val(), 10),
        iStreamType = parseInt($("#record_streamtype").val(), 10),
