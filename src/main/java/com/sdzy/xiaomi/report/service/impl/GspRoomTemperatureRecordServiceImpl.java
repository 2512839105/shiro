...
import com.sdzy.xiaomi.report.mapper.GspRoomTemperatureRecordMapper;
 ...
 */
@Service
public class GspRoomTemperatureRecordServiceImpl extends BaseServiceImpl<GspRoomTemperatureRecordMapper, GspRoomTemperatureRecord> implements GspRoomTemperatureRecordService {
	  
	@Override
	public  Page<GspRoomTemperatureRecord>  selectModelsByPage(QueryParams queryParams) {
		//查询条件构造器
		...
		Page<GspRoomTemperatureRecord> page = new Page<GspRoomTemperatureRecord>(queryParams.getCurrent(), queryParams.getLimit(),queryParams.getSort(), queryParams.isAsc());
		 
		Wrapper<GspRoomTemperatureRecord> wrapper = MyBatisPlusUtil.filtersToWrapper(queryParams.getFilters());
	...
	@Override
	public boolean delectByrecordId(Serializable recordId,String js_code,String appid,String secrect) {
	    try {
			if(BaseUtil.getSessionAndOpenJson(js_code, appid, secrect)){		
	...
	@Override
	public GspRoomTemperatureRecord updateByrecordId( String data,String js_code,String appid,String secrect) {
		try {
				
			if(BaseUtil.getSessionAndOpenJson(js_code, appid, secrect)){
			
			GspRoomTemperatureRecord model=JSON.parseObject(data,GspRoomTemperatureRecord.class);
			//TODO:设置需要修改的字段
